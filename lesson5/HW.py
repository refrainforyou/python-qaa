...
searched_cars = {}
for index, values in car_data.items():
    if values[1] >= search_criteria[0] and values[2] >= search_criteria[1] and values[4] <= search_criteria[2]:
        searched_cars[index] = values

sorted_searched_cars = dict(sorted(searched_cars.items(), key=lambda item: item[1][4]))

while len(sorted_searched_cars) > 5:
    sorted_searched_cars.popitem()
print(sorted_searched_cars)
